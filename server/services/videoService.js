...
exports.downloadVideo = (url, jobId) => {
  return new Promise((resolve, reject) => {
    ...
    exec(`yt-dlp --cookies-from-browser chrome --js-runtimes node -f "bestvideo[ext=mp4]+bestaudio[ext=m4a]/best[ext=mp4]/best" --merge-output-format mp4 -o "${outputTemplate}" "${url}"`, (error, stdout, stderr) => {
      if (error) {
